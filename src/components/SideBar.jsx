...
const SideBar = () => {
    ...
    return (
        <aside >
            <div className="min-md:hidden pb-4">
                <div className="border-solid border-x-0 border-2 border-gray-200  overflow-x-auto">
                    <ul className="py-2 inline-flex">
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isSearchPath ? "bg-gray-100" : ""}`}><Link to={isSearchPath ? `/search?query=${query}&page=${pageNumber}` : `/search?query=${query}`}>All</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isMoviesPath ? "bg-gray-100" : ""}`}><Link to={isMoviesPath ? `/search/movie?query=${query}&page=${pageNumber}` : `/search/movie?query=${query}`}>Movies</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isTVShowPath ? "bg-gray-100" : ""}`}><Link to={isTVShowPath ? `/search/tv?query=${query}&page=${pageNumber}` : `/search/tv?query=${query}`}>TV</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isCollectionPath ? "bg-gray-100" : ""}`}><Link to={isCollectionPath ? `/search/collection?query=${query}&page=${pageNumber}` : `/search/collection?query=${query}`}>Collection</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isAnimePath ? "bg-gray-100" : ""}`}><Link to={isAnimePath ? `/search/anime?query=${query}&page=${pageNumber}` : `/search/anime?query=${query}`}>Anime</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isPeoplePath ? "bg-gray-100" : ""}`}><Link to={isPeoplePath ? `/search/people?query=${query}&page=${pageNumber}` : `/search/people?query=${query}`}>People</Link></li>
                        <li className={`max-sm:px-3 sm:px-5 py-2 hover:bg-gray-50 ${isCompanyPath ? "bg-gray-100" : ""}`}><Link to={isCompanyPath ? `/search/company?query=${query}&page=${pageNumber}` : `/search/company?query=${query}`}>Company</Link></li>
                    </ul>
                </div>
            </div>
            <div className="mr-10 max-md:hidden sticky top-24 h-fit">
                <div className=" w-64">
